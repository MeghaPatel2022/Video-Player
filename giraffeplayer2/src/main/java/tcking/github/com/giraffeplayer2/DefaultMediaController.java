...
public class DefaultMediaController extends BaseMediaController {
    ...
    protected ImageView imgLock, imgBackward, imgForward, app_video_fullscreen;
        ...
        public void onClick(View v) {
            ...
            if (v.getId() == R.id.app_video_fullscreen) {
                ...
                if (aspectRatio == VideoInfo.AR_ASPECT_WRAP_CONTENT) {
                    ...
                    app_video_fullscreen.setImageResource(R.drawable.ic_fit_screen);
            ...
            } else if (v.getId() == R.id.imgLock) {
                if (isLock) {
                    isLock = false;
                    showBottomControl(true);
                    imgLock.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_unlock));
                } else {
                    isLock = true;
                    showBottomControl(false);
                    imgLock.setImageDrawable(context.getResources().getDrawable(R.drawable.ic_lock));
                }
            } else if (v.getId() == R.id.imgForward) {
                LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(context.getApplicationContext());
                Intent localIn;
                localIn = new Intent("TAG_FORWARD");
                lbm.sendBroadcast(localIn);
            } else if (v.getId() == R.id.imgBackward) {
                LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(context.getApplicationContext());
                Intent localIn;
                localIn = new Intent("TAG_BACKWARD");
                lbm.sendBroadcast(localIn);
            }
        }
    };
    ...
    protected void showBottomControl(boolean show) {
        ...
        if (isLock) {
            ...
            $.id(R.id.imgLock).visibility(View.VISIBLE);
            LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(context.getApplicationContext());
            ...
            Intent localIn;
            if (show) {
                localIn = new Intent("TAG_SHOW");
            } else {
                localIn = new Intent("TAG_HIDE");
            }
            lbm.sendBroadcast(localIn);
            $.id(R.id.imgLock).visibility(show ? View.VISIBLE : View.GONE);
...
//            $.id(R.id.app_video_fullscreen).invisible();
    ...
    protected void initView(View view) {
        ...
        imgLock = $.id(R.id.imgLock).view();
        $.id(R.id.imgLock).clicked(onClickListener);

        imgBackward = $.id(R.id.imgBackward).view();
        $.id(R.id.imgBackward).clicked(onClickListener);

        imgForward = $.id(R.id.imgForward).view();
        $.id(R.id.imgForward).clicked(onClickListener);
...
//                        play(url);
    ...
    private void onVolumeSlide(float percent) {
        ...
        // 显示
        $.id(R.id.app_video_volume_icon).image(i == 0 ? R.drawable.ic_sound_off : R.drawable.ic_sound_on);
        $.id(R.id.app_video_brightness_box).gone();
